...
$event = new event();
$min = round($event->min_price('menu') - 1, 0, PHP_ROUND_HALF_UP);
$max = round($event->max_price('menu') + 1, 0, PHP_ROUND_HALF_DOWN);
$from = $min;
$to = $max;
if (!isset($_GET['criteria'])) {
    ...
    switch ($_GET['criteria']) {
        ...
        case 2:
            ...
            $from = $from_price;
            $to = $to_price;
            $menus = $event->read_menus_with_price_range($from_record_num, $records_per_page, $from_price, $to_price);
            $row_count_menus = $event->row_count_menus_with_price_range($from_price, $to_price);
            break;
    }
}